package com.hudhud_sdks.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw HudhudSdksInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw HudhudSdksInvalidDataException("`$name` is not set")
            is JsonNull -> throw HudhudSdksInvalidDataException("`$name` is null")
            else -> throw HudhudSdksInvalidDataException("`$name` is invalid, received $this")
        }
